...
def build_pdf_report(raw_df, meta, gwl, gwl_method_note, area_ratio, nkt, min_thickness=1.0, smooth_window=25,
    ...
    story.append(Paragraph(
        ...
        f"S={shansep_s:.2f}, m={shansep_m:.2f} (patrz strona 1 – założenia). K0 (OC) wg Kulhawy-Mayne "
        "(uwzględnia przekonsolidowanie); K0 (Jaky, NC) – patrz tabela wyżej, dotyczy stanu normalnie "
        "skonsolidowanego i wszystkich gruntów.", styles["Smallr"]))
    story.append(Spacer(1, 4))
    ...
    for _, r in layers.iterrows():
        ...
        rows3.append([
            ...
            fmt(r["OCR (SHANSEP) [-] śr"]), fmt(r["sigma_p (SHANSEP) [kPa] śr"]), fmt(r["Ko [-] śr"]),
        ])
    t3lay = Table([[P(c, "CellSmallHead") for c in rows3[0]]] + [[P(c, "CellSmall") for c in row] for row in rows3[1:]],
                  repeatRows=1, colWidths=[14*mm, 14*mm, 55*mm, 18*mm, 22*mm, 22*mm, 22*mm, 26*mm, 18*mm])
    ...
    formulas = [
        ...
        ("Współczynnik parcia bocznego Ko (OC)", "Ko ≈ 0,5·√OCR", "Kulhawy i Mayne (1990)"),
