...
def build_pdf_report(raw_df, meta, gwl, gwl_method_note, area_ratio, nkt, min_thickness=1.0, smooth_window=25,
    ...
    assum_data = [
        ...
        ["Parametry SHANSEP S, m (do OCR i σ'p)", f"S={shansep_s:.2f}, m={shansep_m:.2f} (typowe wg Ladd 1991)",
         "OCR = (su/(S·σ'v0))^(1/m) – wartości domyślne z literatury dla \"jednorodnych glin o niskiej-średniej "
         "wrażliwości\". W literaturze S i m wahają się mocno wg rodzaju gruntu (S: 0,16–0,48; m: 0,12–0,91) – "
         "bez badań laboratoryjnych na próbkach z tej lokalizacji to tylko orientacja."],
    ...
    for _, r in layers.iterrows():
        ...
        rows.append([
            ...
            fmt3(r["K0 (Jaky, NC) [-] śr"]), fmt_k(r["k [m/s] śr"]),
            r.get("Uwaga", ""),
        ])
    t2 = Table([[P(c, "CellSmallHead") for c in rows[0]]] + [[P(c, "CellSmall") for c in row] for row in rows[1:]],
               repeatRows=1, colWidths=[12*mm, 12*mm, 12*mm, 40*mm, 16*mm, 13*mm, 15*mm, 13*mm, 13*mm, 15*mm, 16*mm, 16*mm, 26*mm])
    ...
    # === TABELA WARSTW - WYTRZYMAŁOŚĆ I HISTORIA NAPRĘŻEŃ (grunty spoiste) ===
    story.append(Paragraph("Wytrzymałość i historia naprężeń – grunty spoiste (Ic > 2,60)", styles["H2r"]))
    story.append(Paragraph(
        f"OCR i σ'p wyznaczone trzema niezależnymi metodami do porównania. SHANSEP liczony z parametrami "
        f"S={shansep_s:.2f}, m={shansep_m:.2f} (patrz strona 1 – założenia). K0 (NC) wg Jaky'ego (1944) i "
        "K0 (OC) wg Kulhawy-Mayne'a (1990, uwzględnia przekonsolidowanie) pokazane obok siebie do porównania "
        "– K0 (NC) jest też w tabeli wyżej dla wszystkich warstw (dotyczy każdego gruntu), tu powtórzony "
        "tylko dla gruntów spoistych, żeby nie zaglądać do dwóch tabel naraz.", styles["Smallr"]))
    story.append(Spacer(1, 4))
    ...
    for _, r in layers.iterrows():
        def fmt(v):
            return "–" if pd.isna(v) else f"{v:.1f}"
        rows3.append([
            f"{r['Od [m]']:.2f}", f"{r['Do [m]']:.2f}", r["Opis"],
            fmt(r["Su [kPa] śr"]), fmt(r["OCR (Robertson 2009) [-] śr"]), fmt(r["OCR (Kulhawy-Mayne) [-] śr"]),
            fmt(r["OCR (SHANSEP) [-] śr"]), fmt(r["sigma_p (SHANSEP) [kPa] śr"]),
            fmt(r["K0 (Jaky, NC) [-] śr"]), fmt(r["Ko [-] śr"]),
        ])
    t3lay = Table([[P(c, "CellSmallHead") for c in rows3[0]]] + [[P(c, "CellSmall") for c in row] for row in rows3[1:]],
                  repeatRows=1, colWidths=[13*mm, 13*mm, 48*mm, 16*mm, 20*mm, 20*mm, 20*mm, 24*mm, 18*mm, 18*mm])
    t3lay.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#2F5597")),
        ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
        ("GRID", (0, 0), (-1, -1), 0.4, colors.HexColor("#DDDDDD")),
        ("ALIGN", (0, 0), (-1, -1), "CENTER"),
    ...
    formulas = [
        ...
        ("OCR (SHANSEP)", "su/σ'v0 = S·(OCR)^m  =>  OCR = (su/(S·σ'v0))^(1/m)", "Ladd i Foott (1974)"),
        ("Naprężenie prekonsolidacji σ'p", "σ'p = σ'v0 · OCR (liczone dla każdej z 3 metod OCR)", "-"),
        ("Współczynnik parcia bocznego K0 (NC)", "K0 = 1 − sin(φ')", "Jaky (1944)"),
        ("Współczynnik parcia bocznego K0 (OC)", "K0 ≈ 0,5·√OCR", "Kulhawy i Mayne (1990)"),
