...
for uf in uploaded_files:
    ...
    with st.expander(f"📄 {uf.name}", expanded=(len(uploaded_files) == 1)):
        ...
        test_number = c1.text_input("Numer testu", "", key=f"tn_{uf.name}")
        cone_number = c2.text_input("Nr stożka", "", key=f"cn_{uf.name}")
        date_str = c3.text_input("Data", "", key=f"dt_{uf.name}")
        investor = c4.text_input("Inwestor", "", key=f"inv_{uf.name}")
        ...
        area_ratio = c2.number_input("Współcz. powierzchni stożka (a)", value=0.80, min_value=0.5,
                                      max_value=1.0, step=0.01, key=f"a_{uf.name}")
        ...
        if st.button("🔄 Przelicz i pokaż podgląd", key=f"btn_{uf.name}"):
            with st.spinner("Liczę interpretację geotechniczną..."):
                ...
                st.session_state[f"report_{uf.name}"] = (pdf_buf.getvalue(), profile, layers)

        cached = st.session_state.get(f"report_{uf.name}")
        if cached:
            ...
            st.download_button(
                ...
                mime="text/csv",
                key=f"dlcsv_{uf.name}",
            )

# --- pobranie wszystkich naraz, jesli jest wiecej niz 1 plik i wszystkie przeliczone ---
all_ready = [f.name for f in uploaded_files if st.session_state.get(f"report_{f.name}")]
if len(uploaded_files) > 1 and all_ready:
    ...
    with zipfile.ZipFile(zip_buf, "w") as zf:
        for f in uploaded_files:
            cached = st.session_state.get(f"report_{f.name}")
            if not cached:
                continue
            pdf_bytes, profile, layers = cached
