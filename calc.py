...
def compute_profile(df, gwl=0.0, area_ratio=0.8, nkt=15.0, gamma_init=18.0,
    ...
    shansep_s, shansep_m: parametry SHANSEP (Ladd i Foott, 1974) do wyznaczenia OCR
        z su/sigma'v0 = S*(OCR)^m. Domyślne 0.22/0.80 to typowe wartości literaturowe
        (Ladd 1991, "homogeneous clays of low-moderate sensitivity") - BEZ lokalnej
        kalibracji badaniami laboratoryjnymi to wartości orientacyjne, mogą się mocno
        różnić dla konkretnego gruntu (literatura pokazuje S: 0.16-0.48, m: 0.12-0.91
    ...
    qt = np.clip(qt, 0.001, None)  # zabezpieczenie: u2 lekko ujemne (szum blisko zera) mogło dać qt<=0
    ...
    # OCR wg SHANSEP (Ladd i Foott, 1974): su/sigma'v0 = S*(OCR)^m  =>  OCR = (su/(S*sigma'v0))^(1/m)
    OCR_shansep = np.where(fine, (Su_ratio / shansep_s) ** (1.0 / shansep_m), np.nan)

    # naprezenie prekonsolidacji sigma'p = sigma'v0 * OCR, wg kazdej z 3 metod OCR
    sigma_p_robertson = np.where(fine, sigma_v0 * OCR_robertson, np.nan)
    sigma_p_km = np.where(fine, sigma_v0 * OCR_km, np.nan)
    sigma_p_shansep = np.where(fine, sigma_v0 * OCR_shansep, np.nan)

    # K0 - dwa warianty:
    # 1) Jaky (1944), grunt normalnie skonsolidowany (NC): K0 = 1 - sin(phi')
    K0_nc = 1.0 - np.sin(np.radians(phi))
    # 2) Kulhawy-Mayne (1990), skorygowane o OCR (grunt prekonsolidowany):
    #    K0_OC = K0_NC * OCR^sin(phi')  (uproszczenie stosowane w praktyce: 0.5*sqrt(OCR) ponizej)
    ...
    out = pd.DataFrame({
        ...
        "OCR (SHANSEP) [-]": OCR_shansep,
        "sigma_p (Robertson) [kPa]": sigma_p_robertson, "sigma_p (Kulhawy-Mayne) [kPa]": sigma_p_km,
        "sigma_p (SHANSEP) [kPa]": sigma_p_shansep,
        "K0 (Jaky, NC) [-]": K0_nc,
        "Ko [-]": Ko, "k [m/s]": k_perm,
    })
    return out
